...
#             invoke_url = "https://gldssox2ei.execute-api.us-east-1.amazonaws.com/kinesis_API"
...
def run_infinite_post_data_loop():
    while True:
        ...
        with engine.connect() as connection:
            ...
            headers = {'Content-Type': 'application/x-amz-json-1.1'}
            response_pin = requests.request("POST", f"{base_invoke_url}{stream_name_pin}", headers=headers, data=payload_pin)
            response_geo = requests.request("POST", f"{base_invoke_url}{stream_name_geo}", headers=headers, data=payload_geo)
            response_user = requests.request("POST", f"{base_invoke_url}{stream_name_user}", headers=headers, data=payload_user)
